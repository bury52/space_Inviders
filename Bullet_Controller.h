...
public:
    class Buller_Helper {
    public:
        explicit Buller_Helper(Bullet_Controller<Target...> *const self, std::optional<Target> ... targets)
    ...
    private:
        ...
        std::tuple<std::optional<Target>...> targets;
    };

    Buller_Helper get_helper(std::optional<Target>... targets) {
    ...
    void update(const sf::Time &elapsed) {
        ...
        erase_if(bullets, [&](Bullet_Wraperr &wraperr) {
            ...
            if ((wraperr.bullet.turn == TurnState::Up && wraperr.bullet.get_bounds().position.y < 0) || (wraperr.bullet.turn == TurnState::Down && wraperr.bullet.get_bounds().position.y > 1000))
                return true;
